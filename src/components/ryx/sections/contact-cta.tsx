...
import { makeStaggerParent, FADE_UP_CHILD, SLIDE_LEFT_CHILD, SLIDE_RIGHT_CHILD } from "../motion";
...
const splitParent = makeStaggerParent(0.15, 0.2);
...
export function ContactCTASection() {
  ...
  return (
    <ParallaxBanner className="ig-section-dark">
      {/* Layer 1 — texture drifts fastest */}
      <ParallaxBannerLayer
        translateY={[-22, 22]}
        scale={[1.08, 1]}
      ...
      {/* Layer 2 — gradient overlay drifts slightly slower */}
      <ParallaxBannerLayer
        translateY={[-8, 8]}
        shouldAlwaysCompleteAnimation
        className="bg-gradient-to-b from-ig-dark via-ig-dark-card/60 to-ig-dark pointer-events-none"
      />

      {/* Content drives the banner height */}
      <div className="relative z-10 py-20 sm:py-28 lg:py-36 overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">

          <SectionReveal variants={headerParent} amount={0.2}>
            <motion.div variants={FADE_UP_CHILD} className="mb-6">
              <SectionLabel text="Start the conversation today" variant="dark" />
            </motion.div>
            <motion.div variants={FADE_UP_CHILD} className="mb-14 sm:mb-20">
              <CinematicHeading className="ig-heading-1 max-w-3xl" variant="dark" delay={0}>
                Start your Project today
              </CinematicHeading>
            </motion.div>
          </SectionReveal>

          <SectionReveal variants={splitParent} className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16" amount={0.1}>

            {/* Left — form slides from left */}
            <motion.div variants={SLIDE_LEFT_CHILD}>
              <form onSubmit={handleSubmit} className="space-y-5">
                <div>
                  <label htmlFor="cta-name" className="block text-xs text-ig-text-light-muted mb-1.5">Full name *</label>
                  <input id="cta-name" name="name" type="text" required value={form.name} onChange={handleChange}
                ...
                <div>
                  <label htmlFor="cta-email" className="block text-xs text-ig-text-light-muted mb-1.5">Email address *</label>
                  <input id="cta-email" name="email" type="email" required value={form.email} onChange={handleChange}
                    className="w-full bg-ig-white-5 border border-ig-white-10 rounded-xl px-4 py-3 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-white/30 transition-colors"
                    placeholder="john@company.com" />
                </div>
                <div>
                  <label htmlFor="cta-message" className="block text-xs text-ig-text-light-muted mb-1.5">Message</label>
                  <textarea id="cta-message" name="message" rows={4} value={form.message} onChange={handleChange}
                ...
                {status === "success" ? (
                  <div className="inline-flex items-center gap-2.5 rounded-full bg-ig-green/20 text-ig-green px-6 py-3 text-sm font-medium">
                    <Check size={14} /><span>Message sent!</span>
                  </div>
                ) : (
                  <button type="submit" disabled={status === "loading"}
                    ...
                    {status === "loading" ? <><span>Sending…</span><Send size={14} className="animate-pulse" /></> : <><span>Submit</span><Send size={14} /></>}
                  </button>
                )}
                {status === "error" && <p className="mt-2 text-xs text-red-400">Something went wrong. Please try again or email us directly.</p>}
              </form>
              <div className="mt-8 flex items-center gap-3">
                ...
                <div>
                  ...
                  <button onClick={copyEmail} className="flex items-center gap-2 text-sm font-medium hover:text-white/80 transition-colors cursor-pointer">
                    <span>{SITE_CONFIG.company.email}</span>
                    {copied ? <Check size={14} className="text-ig-green" /> : <Copy size={14} className="text-white/40" />}
                  </button>
                </div>
              </div>
            </motion.div>
            ...
            <motion.div variants={SLIDE_RIGHT_CHILD} className="space-y-8">
              <div>
