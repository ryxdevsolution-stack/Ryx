...
void main() {
  ...
  if (uAutoCenterRepulsion > 0.0) {
    ...
    uv += repulsion * 0.07 * uMouseActiveFactor;
  } else {
...
export default function Galaxy({
  ...
  // Fix 2 & 4 — Stable refs for props used inside RAF/resize closures.
  // These sync on every render without re-triggering the WebGL init effect.
  ...
  useEffect(() => { disableAnimationRef.current = disableAnimation; }, [disableAnimation]);
  ...
  useEffect(() => {
    ...
    schedule(async () => {
      ...
      // Dynamic import — OGL stays out of the initial JS parse budget
      const { Renderer, Program, Mesh, Vec3, Triangle } = await import('ogl');
      if (isDestroyed) return;
      ...
      const resolution = new Vec3(gl.canvas.width, gl.canvas.height, gl.canvas.width / gl.canvas.height);
      ...
      const program = new Program(gl, {
        ...
        uniforms: {
          ...
          uStarSpeed: { value: starSpeedRef.current },
      ...
      function update(t: number) {
        ...
        if (!disableAnimationRef.current) {
          ...
          program.uniforms.uStarSpeed.value = (t * 0.001 * starSpeedRef.current) / 10.0;
        }
        ...
        smoothMouseActive.current += (targetMouseActive.current - smoothMouseActive.current) * activeLerp;
      ...
      // Store cleanup fns so outer cleanup can call them even after async resolves
      cleanupRef.current = () => {
        isDestroyed = true;
    ...
    density, hueShift, mouseInteraction, glowIntensity, saturation, mouseRepulsion,
